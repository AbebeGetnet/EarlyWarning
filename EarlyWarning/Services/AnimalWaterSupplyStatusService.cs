...
{
    public class AnimalWaterSupplyStatusService
    {
        private readonly IAnimalWaterSupplyStatusRepository _repository;
        private readonly UserManager<ApplicationUser> _userManager;

        public AnimalWaterSupplyStatusService(IAnimalWaterSupplyStatusRepository repository, UserManager<ApplicationUser> userManager)
        {
            _repository = repository;
            _userManager = userManager;
        }

        public async Task<IEnumerable<AnimalWaterSupplyStatus>> GetAllReportsAsync()
        {
            return await _repository.GetAllAsync(r => r.Woreda);
        }

        public async Task<AnimalWaterSupplyStatus?> GetReportByIdAsync(Guid id)
        {
            return await _repository.GetByIdAsync(id, r => r.Woreda);
        }

        public async Task CreateReportAsync(AnimalWaterSupplyStatus report)
        {
            await _repository.AddAsync(report);
            await _repository.SaveChangesAsync();
        }

        public async Task UpdateReportAsync(AnimalWaterSupplyStatus report)
        {
        ...
        public async Task SaveReportAsync(AnimalWaterSupplyStatus report)
        {
            _repository.Update(report);
            await _repository.SaveChangesAsync();
        }
